...
class ProfileHandler(BaseHandler):
    ...
    def get_notifications(self):
        notifications = Notification.query(
            Notification.profile == self.logged.key).order(-Notification.created_at).fetch(5)

        return self.render_json({'notifications': [n.to_dict() for n in notifications]})

    def add_notification(self):
        notify = Notification(
            text=self.request.get('text'), profile=self.logged.key, ntype='icon-comment')
        notify.put()
