...
function avatar_editable(){
	try {//ie8 throws some harmless exception, so let's catch it
		...
		$('#avatar').editable({
				...
				on_error : function(code) {//on_error function will be called when the selected file has a problem
					...
					if(code == 1) {//file format error
						last_gritter = $.gritter.add({
							...
							text: 'Por favor seleciona uma imagem nos formatos jpg|gif|png !',
							class_name: 'gritter-error gritter-center'
						});
					} else if(code == 2) {//file size rror
						last_gritter = $.gritter.add({
							title: 'File too big!',
							text: 'Tamanho da imagem deve ser menor que 2Mb!',
							class_name: 'gritter-error gritter-center'
						});
					}
					else {//other error
			...
			url: function(params) {
				var submit_url = $.profile_avatar_url;
				var deferred;
				...
				if( !("FormData" in window) ) {
					...
					deferred = $.ajax({
						...
						error: function(err){
							last_gritter = $.gritter.add({
							title: 'Erro durante upload do arquivo!',
							text: '',
							class_name: 'gritter-error gritter-center'
						});
							setTimeout(function(){
				...
				deferred.done(function(res){
					if(res.status === 'OK'){
						$('#avatar').get(0).src = res.url + '=s200';
						$('#avatar-nav').attr('src', res.url+'=s45');
					}
				}).fail(function(res){
					console.warn('def fail');
					console.warn(res);
				});
